...
-(void)request:(ACWebDAVUploadRequest*)request didUploadItem:(ACWebDAVItem*)item {
    ...
    [self showAlert:[NSString stringWithFormat:@"File upload succeeded"]:[NSString stringWithFormat:@""]];
}
...
-(void)request:(id)request didFailWithErrorCode:(int)errorCode {
	...
	if([request isKindOfClass:[ACWebDAVUploadRequest class]]) {
        ...
        {
            [self showAlert:[NSString stringWithFormat:@"File upload failed"]:[NSString stringWithFormat:@"ErrorCode: %d", errorCode]];
        }
		return;
	}
